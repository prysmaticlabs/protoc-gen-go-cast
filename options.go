...
// Tools from: https://github.com/Fanatics/protobuf-ast-helpers
...
func (o *OptionReader) readValue(in interface{}, out interface{}) error {
	...
	case *string:
		data, ok := in.(string)
		if !ok {
			return fmt.Errorf("value was %T, not %T", in, data)
		}
		*target = data
	case *bool:
		data, ok := in.(bool)
	...
	case *int64:
		...
		*target = data
	default:
		return fmt.Errorf("helper could not read %T, please use `GetOption...` directly", target)
	}
